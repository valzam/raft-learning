...
func TestReElection2A(t *testing.T) {
	...
	println("Dropped first leader")
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	println("Rejoined first leader")
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	println("Dropped majority of servers")
	toDrop1 := leader2
	toDrop2 := (leader2 + 1) % servers
	fmt.Printf("Dropping %d and %d\n", toDrop1, toDrop2)
	cfg.disconnect(toDrop1)
	cfg.disconnect(toDrop2)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// // if a quorum arises, it should elect a leader.
	println("Reconnect one server to allow election")
	cfg.connect(toDrop2)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	println("Rejoin old leader")
	cfg.connect(toDrop1)
	cfg.checkOneLeader()
