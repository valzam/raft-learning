...
func TestReElection2A(t *testing.T) {
	...
	println("Test (2A): disconnect leader")
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	println("Test (2A): rejoin old leader")
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	println("Test (2A): disconnect 2/3, no quorum")
	...
	println("Test (2A): Reconnect one server to allow quorum")
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	println("Test (2A): Reconnect last server, should convert to follower")
	cfg.connect(leader2)
	cfg.checkOneLeader()
